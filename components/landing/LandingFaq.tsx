...
const FAQ_ITEMS: FaqItem[] = [
  {
    ...
    answer:
      'En scannant votre QR code ou en cliquant sur votre lien, vos interlocuteurs appuient sur le bouton « Ajouter aux contacts ». Un fichier universel vCard (.vcf) est généré instantanément et s’ouvre directement dans le carnet d’adresses de leur smartphone (iPhone Contacts ou Google Contacts) avec votre nom, prénom, photo, téléphone, WhatsApp, email et réseaux pré-remplis sans aucune faute de frappe.',
  },
  {
  ...
  },
  {
    ...
    answer:
      'Non ! Lien-Bio propose une formule 100% Gratuite pour démarrer. Pour les professionnels, nous proposons également un accès annuel économique (-38%) ainsi qu’un Pack PRO À Vie à paiement unique définitif (vous ne payez qu’une seule fois et bénéficiez de toutes les fonctionnalités pour toujours, sans aucun prélèvement futur).',
  },
];
      ...
      {FAQ_ITEMS.map((item, idx) => {
        ...
        return (
          ...
          >
            ...
            >
              ...
              <div
                className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 transition-transform duration-200 ${
                  isOpen ? 'bg-indigo-50 text-indigo-600 rotate-180' : 'bg-neutral-100 text-neutral-500'
                }`}
              >
            ...
            {isOpen && (
              <div className="px-4 sm:px-5 pb-5 pt-0 text-xs sm:text-sm text-neutral-600 leading-relaxed animate-in fade-in duration-200 border-t border-neutral-100/80 mt-1">
                {item.answer}
              </div>
            )}
          </div>
